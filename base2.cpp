...
int main()
{
  //  std::thread Ant(Anti_Debug);
    ...
    std::string i2;
    FILE* file;
    if (file = fopen("key.txt", "r")) {

        std::string key;
        std::ifstream InFile("key.txt", std::ios::in);
    ...
    Options::login::login3 = true;
	if (!KeyAuthApp.data.complete)
	{
		KeyAuthApp.log(TEXT("") + KeyAuthApp.data.message);
	...
	{
		Options::login::login2 = true;
	}
	if (check)
	{
		Options::login::login4 = true;
	}
    ...
    if (KeyAuthApp.data.expiry == "99999999999999")
    {


        exit(-1);
    }
    if (KeyAuthApp.data.expiry == "1337")
    {


        exit(-1);
    }
    if (KeyAuthApp.data.hwid == "-_-")
    {


        exit(-1);
    }
    if (KeyAuthApp.data.hwid == "")
    {


        exit(-1);
    }
    if (KeyAuthApp.data.createdate == "0")
    {
    ...
    //CreateThread(nullptr, NULL, (LPTHREAD_START_ROUTINE)checkvalid, nullptr, NULL, nullptr);


    if (MessageBoxA(NULL, XorStr("Enable Streamproof?").c_str(), XorStr("").c_str(), MB_YESNO) == IDYES)
    {
       streamproof = true;
    }
    else
    ...
    Init();
	system(TEXT("cls"));
	std::cout << TEXT("[~] Waiting for Fortnite...") << endl;
    std::cout << TEXT("[~] Press F8 To Exit.") << endl;
    ...
    if (!communication->attach(process_id))
    {
	...
	virtualaddy = communication->get_process_base();

	if (virtualaddy == 0)
		std::cout << TEXT(" Failed to find Base address... ") << endl;

	Overlay::BeginOverlay();
	if (!OverlayWindow) {

		cout << TEXT (" Overlay Failed") << endl;
		Sleep(2500);
		exit(0);
	}
