...
namespace Overlay {
	...
	void KeyLoop() {
		...
		if (GetAsyncKeyState(VK_INSERT) & 1) {
			...
			glfwSetWindowAttrib(OverlayWindow, GLFW_MOUSE_PASSTHROUGH, !Options::ShowMenu);
			if (Options::ShowMenu) {
				HWND overlay_window = glfwGetWin32Window(OverlayWindow);
				SetForegroundWindow(overlay_window);
			}
			else {
				//SetForegroundWindow(fn_window);
			}
		}
